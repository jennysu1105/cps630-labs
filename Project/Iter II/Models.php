...
class Truck implements Database
{
    public $truck_id;
    public $truck_code;
    ...
    {
        if ($this->truck_id != null) {
            $updateTruck = "UPDATE truckTable SET truck_id=$this->truck_id, truck_code=$this->truck_code, availability_code=$this->availability_code WHERE truck_id=$this->truck_id";
...
class Trip implements Database
{
    public $trip_id;
    public $source_code;
    ...
    {
        if ($this->trip_id != null) {
            $updateTrip = "UPDATE tripTable SET trip_id=$this->trip_id, source_code='$this->source_code', destination_code='$this->destination_code', distance=$this->distance, truck_id=$this->truck_id, price=$this->price WHERE trip_id=$this->trip_id";
...
class User implements Database
{
    ...
    {
        if ($this->user_id != null) {
            $updateUser = "UPDATE userTable SET user_id=$this->user_id, full_name='$this->full_name', telephone='$this->telephone', email='$this->email', home_address='$this->home_address', city_code='$this->city_code', login_id='$this->login_id', user_password='$this->user_password', balance=$this->balance  WHERE user_id=$this->user_id";
...
class Item implements Database
{
    public $item_id;
    public $item_name;
    public $item_price;
    ...
    {
        if ($this->item_id != null) {
            $updateItem = "UPDATE itemTable SET item_id=$this->item_id, item_name='$this->item_name', item_price=$this->item_price, made_in='$this->made_in', department_code='$this->department_code' WHERE item_id=$this->item_id";
            return submitQuery($updateItem);
        }
    }
...
class Order implements Database
{
    public $order_id;
    public $date_issued;
    ...
    /**
     * Enter 7 parameter ($date_issued, $date_received, $total_price, $payment_code, $user_id, $trip_id, $receipt_id) for creating the object from user input.
     * Enter 8 parameter ($order_id, $date_issued, $date_received, $total_price, $payment_code, $user_id, $trip_id, $receipt_id) for creating the obejct from database.
     */
    public function __construct()
    {
        $arguments = func_get_args();
    ...
    public function __construct7($date_issued, $date_received, $total_price, $payment_code, $user_id, $trip_id, $receipt_id)
    {
    ...
    public function __construct8($order_id, $date_issued, $date_received, $total_price, $payment_code, $user_id, $trip_id, $receipt_id)
    {
    ...
    public function insert()
    {
        if ($this->order_id == null) {
            $insertOrder = "INSERT INTO orderTable (date_issued, date_received, total_price, payment_code, user_id, trip_id, receipt_id) VALUES ('$this->date_issued', '$this->date_received', $this->total_price, '$this->payment_code', $this->user_id, $this->trip_id, $this->receipt_id)";
    ...
    {
        if ($this->order_id != null) {
            $updateOrder = "UPDATE orderTable SET order_id=$this->order_id, date_issued='$this->date_issued', date_received='$this->date_received', total_price=$this->total_price, payment_code='$this->payment_code', user_id=$this->user_id, trip_id=$this->trip_id, receipt_id=$this->receipt_id WHERE order_id=$this->order_id";
